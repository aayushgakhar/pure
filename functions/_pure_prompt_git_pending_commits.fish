function _pure_prompt_git_pending_commits --argument-names ahead behind
    ...
    if test -n "$has_upstream"  # check there is an upstream repo configured
        ...
        if test "$commit_to_push" -gt 0  # upstream is behind local repo
            ...
            set git_unpushed_commits ' '$pure_symbol_git_unpushed_commits$ahead
            echo "$git_unpushed_commits_color$git_unpushed_commits"
        ...
        if test "$commit_to_pull" -gt 0  # upstream is ahead of local repo
            ...
            set git_unpulled_commits ' '$pure_symbol_git_unpulled_commits$behind
            echo "$git_unpulled_commits_color$git_unpulled_commits"
        end
    end
end
